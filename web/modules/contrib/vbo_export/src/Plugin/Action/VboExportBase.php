...
abstract class VboExportBase extends ViewsBulkOperationsActionBase implements ContainerFactoryPluginInterface {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, StreamWrapperManagerInterface $streamWrapperManager, PrivateTempStoreFactory $temp_store_factory, TimeInterface $time) {
  ...
  protected function sendToFile($output) {
    if (!empty($output)) {
      ...
      $filename = $this->context['view_id'] . '_' . date('Y_m_d_H_i', $this->time->getRequestTime()) . '-' . $rand . '.' . static::EXTENSION;
      ...
      // Get relative url to prevent mixed content errors when using HTTPS + HTTP.
      $relative_url = file_url_transform_relative(file_create_url($file->getFileUri()));
